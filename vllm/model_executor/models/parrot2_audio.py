...
from transformers.models.parrot_sensevoice import (
    ParrotSenseVoiceEncoder as TransformersParrot2AudioEncoder,
)
from transformers.models.parrot_sensevoice import (
    ParrotSenseVoiceFeatureExtractor as Parrot2AudioFeatureExtractor,
)
from transformers.models.parrot_sensevoice import (
    ParrotSenseVoiceProcessor as Parrot2AudioProcessor,
)
...
# # === Audio Inputs === #
class Parrot2AudioFeatureInputs(TensorSchema):
    """
    Dimensions:
        - na: Number of audios
...
class Parrot2AudioEmbeddingInputs(TensorSchema):
    """
    Dimensions:
        - bn: Batch size
...
Parrot2AudioInputs: TypeAlias = Parrot2AudioFeatureInputs | Parrot2AudioEmbeddingInputs
...
class Parrot2AudioProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs: object) -> Parrot2AudioProcessor:
        return self.ctx.get_hf_processor(Parrot2AudioProcessor, **kwargs)

    def get_feature_extractor(self, **kwargs: object) -> Parrot2AudioFeatureExtractor:
...
class Parrot2AudioDummyInputsBuilder(
    BaseDummyInputsBuilder[Parrot2AudioProcessingInfo]
):
...
def _parrot2_audio_field_config(hf_inputs: Mapping[str, torch.Tensor]):
    return dict(
...
class Parrot2AudioMultiModalDataParser(MultiModalDataParser):
    def _parse_audio_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=_parrot2_audio_field_config,
            )

        return super()._parse_audio_data(data)


class Parrot2AudioMultiModalProcessor(
    BaseMultiModalProcessor[Parrot2AudioProcessingInfo]
):
    def _get_data_parser(self) -> MultiModalDataParser:
        feature_extractor = self.info.get_feature_extractor()
        return Parrot2AudioMultiModalDataParser(target_sr=feature_extractor.fs)
    ...
    def _get_mm_fields_config(
        ...
        return _parrot2_audio_field_config(hf_inputs)
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_parrot2_audio(item_idx: int):
        ...
        return [
            PromptReplacement(
                modality="audio",
                target=audio_token,
                replacement=get_replacement_parrot2_audio,
            )
        ]
...
class Parrot2AudioForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        if (
            ...
            raise ValueError(
                "VLLM_COMPILE is not supported for parrot2_audio's mm_encoder, please use DYNAMO_TRACE_ONCE or STOCK_TORCH_COMPILE instead"
            )
        ...
        with set_default_torch_dtype(self.audio_tower_dtype):
            if os.environ.get("VLLM_USE_TRANSFORMERS_AUDIO_ENCODER", "0") == "1":
                self.audio_tower = TransformersParrot2AudioEncoder(config.audio_config)
            else:
                self.audio_tower = Parrot2AudioEncoder(vllm_config=vllm_config)
        ...
        with set_default_torch_dtype(self.audio_tower_dtype):
            if (
                ...
                self.multi_modal_projector = (
                    TransformersParrot2AudioMultiModalProjector(config)
                )
            else:
                self.multi_modal_projector = Parrot2AudioMultiModalProjector(
    ...
    def _parse_and_validate_audio_input(
        ...
        if audio_embeds is not None:
            return Parrot2AudioEmbeddingInputs(
                type="audio_embeds", audio_embeds=audio_embeds
            )

        if input_features is not None:
            return Parrot2AudioFeatureInputs(
                type="audio_features",
    ...
    def _process_audio_input(
        self, audio_input: Parrot2AudioInputs
    ) -> tuple[torch.Tensor]:
