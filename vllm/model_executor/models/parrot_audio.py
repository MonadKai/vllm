...
from transformers.models.parrot_audio import ParrotAudioEncoder, ParrotQwen2ForCausalLM
...
class ParrotAudioForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # TODO: implement a special weight loader for parrot_audio
        self.audio_tower = ParrotAudioEncoder(config.audio_config)
        # HINT: audio tower is loaded and inference in float32 format
        self._audio_dtype = torch.float32
        ...
        # HINT: projector is loaded and inference in float32 format
        self._projector_dtype = torch.float32

        self.quant_config = quant_config

        # HINT: for parrot_audio's language_model, `_tied_weights_keys` is always empty
        ...
        self._language_model_dtype = torch.bfloat16
        # HINT: language_model is loaded in float32 format, but inference in bfloat16 format
        self._convert_language_model_dtype()
        # self._ensure_correct_dtypes()
 
        self.make_empty_intermediate_tensors = (
            self.language_model.make_empty_intermediate_tensors)

    def _convert_language_model_dtype(self):
        if hasattr(self.language_model, 'model'):
            for param in self.language_model.parameters():
                if param.dtype == torch.float32:
                    param.data = param.data.to(self._language_model_dtype)
    ...
    def get_multimodal_embeddings(self, **kwargs) -> Optional[MultiModalEmbeddings]:
        ...
        masked_audio_features = tuple(t.to(self._language_model_dtype) for t in masked_audio_features)
    ...
    def forward(
        self,
        input_ids: torch.Tensor,  # [8192] or [492]
        positions: torch.Tensor,  # [8192] or [492]
        intermediate_tensors: Optional[IntermediateTensors] = None,  # None
    ...
    def _ensure_correct_dtypes(self):
        for param in self.audio_tower.parameters():
            if param.dtype != self._audio_dtype:
                raise ValueError(f"audio_tower param `{param.name}` dtype mismatch: {param.dtype} != {self._audio_dtype}")
        
        for param in self.multi_modal_projector.parameters():
            if param.dtype != self._projector_dtype:
                raise ValueError(f"multi_modal_projector param `{param.name}` dtype mismatch: {param.dtype} != {self._projector_dtype}")
        
        for param in self.language_model.parameters():
            if param.dtype != self._language_model_dtype:
                raise ValueError(f"language_model param `{param.name}` dtype mismatch: {param.dtype} != {self._language_model_dtype}")
