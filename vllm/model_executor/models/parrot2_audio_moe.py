...
from transformers.models.parrot_sensevoice import (
    ParrotSenseVoiceEncoder as TransformersParrot2AudioMoeEncoder,
)
from transformers.models.parrot_sensevoice import (
    ParrotSenseVoiceFeatureExtractor as Parrot2AudioMoeFeatureExtractor,
)
...
# # === Audio Inputs === #
class Parrot2AudioMoeFeatureInputs(TensorSchema):
    """
    Dimensions:
        - na: Number of audios
...
class Parrot2AudioMoeEmbeddingInputs(TensorSchema):
    """
    Dimensions:
        - bn: Batch size
...
Parrot2AudioMoeInputs: TypeAlias = (
    Parrot2AudioMoeFeatureInputs | Parrot2AudioMoeEmbeddingInputs
)
...
class Parrot2AudioMoeDummyInputsBuilder(
    BaseDummyInputsBuilder[Parrot2AudioMoeProcessingInfo]
):
...
def _parrot2_audio_moe_field_config(hf_inputs: Mapping[str, torch.Tensor]):
    return dict(
...
class Parrot2AudioMoeMultiModalDataParser(MultiModalDataParser):
    def _parse_audio_data(
        ...
        if isinstance(data, dict):
            return DictEmbeddingItems(
                ...
                fields_factory=_parrot2_audio_moe_field_config,
            )
...
class Parrot2AudioMoeMultiModalProcessor(
    ...
    def _get_data_parser(self) -> MultiModalDataParser:
        feature_extractor = self.info.get_feature_extractor()
        return Parrot2AudioMoeMultiModalDataParser(target_sr=feature_extractor.fs)
    ...
    def _get_mm_fields_config(
        ...
        return _parrot2_audio_moe_field_config(hf_inputs)
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_parrot2_audio_moe(item_idx: int):
        ...
        return [
            PromptReplacement(
                modality="audio",
                target=audio_token,
                replacement=get_replacement_parrot2_audio_moe,
            )
        ]
...
class Parrot2AudioMoeForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        # HINT: parrot2_audio_moe is a mixed precision model
        ...
        if (
            ...
            raise ValueError(
                "VLLM_COMPILE is not supported for parrot2_audio_moe's mm_encoder, please use DYNAMO_TRACE_ONCE or STOCK_TORCH_COMPILE instead"
            )
        ...
        with set_default_torch_dtype(self.audio_tower_dtype):
            if os.environ.get("VLLM_USE_TRANSFORMERS_AUDIO_ENCODER", "0") == "1":
                self.audio_tower = TransformersParrot2AudioMoeEncoder(
                    config.audio_config
                )
            else:
        ...
        with set_default_torch_dtype(self.audio_tower_dtype):
            if (
                ...
                self.multi_modal_projector = (
                    TransformersParrot2AudioMoeMultiModalProjector(config)
                )
            else:
    ...
    def _parse_and_validate_audio_input(
        ...
        if audio_embeds is not None:
            return Parrot2AudioMoeEmbeddingInputs(
                type="audio_embeds", audio_embeds=audio_embeds
            )

        if input_features is not None:
            return Parrot2AudioMoeFeatureInputs(
                type="audio_features",
    ...
    def _process_audio_input(
        self, audio_input: Parrot2AudioMoeInputs
    ) -> tuple[torch.Tensor]:
