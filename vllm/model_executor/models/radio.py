...
class RadioInternVisionModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.img_size = to_2tuple(config.internvit_config.image_size)

        max_img_size = int(
            round(args.cpe_max_size / config.patch_size) * config.patch_size
        )
        uq_teachers = set(t['name'] for t in args.teachers)

        self.patch_generator = ViTPatchGenerator(
            patch_size=config.patch_size,
            embed_dim=config.internvit_config.hidden_size,
        ...
        self.encoder = InternVisionEncoder(
            config=config.internvit_config,
...
class RadioModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        summary_idxs = torch.tensor([i for i, t in enumerate(args.teachers) if t.get("use_summary", True)])
